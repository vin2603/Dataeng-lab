...
DBname = "postgres"
DBuser = "postgres"
DBpwd = "12345"   # insert your postgres db password here
...
def createTable(conn):
    ...
    with conn.cursor() as cursor:
        cursor.execute(f"""
            ...
            -- DELAYED: no primary key or index for faster loading
...
def load(conn, rlis):
    with conn.cursor() as cursor:
        print(f"Loading {len(rlis)} rows with copy_from...")
        ...
        with open(Datafile, 'r') as f:
            ...
            cursor.copy_from(f, TableName, sep=',', null='', columns=(
